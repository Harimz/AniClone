import { useGetAnimeEpisodesQuery } from "@/redux/features/animeApiSlice";
import { AnimeEpisodes } from "@/types";
import { useState, useEffect } from "react";

const useFetchAnimeStaff = (delay = 0, id: number) => {
  const [data, setData] = useState<AnimeEpisodes | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [retryCount, setRetryCount] = useState(0);

  const maxRetries = 3;

  const response = useGetAnimeEpisodesQuery(id);
