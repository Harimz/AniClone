import { useGetAnimeStaffQuery } from "@/redux/features/animeApiSlice";
import { AnimeStaffData } from "@/types";
import { useState, useEffect } from "react";

const useFetchAnimeStaff = (delay = 0, id: number) => {
  const [data, setData] = useState<AnimeStaffData | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [retryCount, setRetryCount] = useState(0);

  const maxRetries = 3;

  const response = useGetAnimeStaffQuery(id);
